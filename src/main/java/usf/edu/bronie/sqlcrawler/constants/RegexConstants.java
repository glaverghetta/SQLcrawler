...
public class RegexConstants {
        ...
        public static final String CONCAT_VAR_MULTIPLE = WHITESPACE + "(?=" + "%s" + "[_a-zA-Z][_a-zA-Z0-9]*"
                        + WHITESPACE + "\\," + CONCAT_VAR + ")";
        // Parisa:
        // Java:
        // public static final String CONCAT_VAR_MULTIPLE = WHITESPACE + "(?=" + "%s" +
        // "[_a-zA-Z][_a-zA-Z0-9]*" + WHITESPACE + "%s" + CONCAT_VAR + ")";
        // %s: AND. OR
        ...
        public static final String STRING_LITERAL_CONCAT_WITH_VAR_LOWER = "\\\"[^\\\"\\\\]*(\\\\.[^\\\"\\\\]*)*\\\"(?="
                        + WHITESPACE + "\\" + "%s " + WHITESPACE + "%s" + "[a-z][_a-zA-Z0-9]*)";
        ...
        public static final String APPEND = "append" + WHITESPACE + "\\(" + WHITESPACE + "[_a-zA-Z][_a-zA-Z0-9]*"
                        + WHITESPACE + "\\)";
        ...
        // "\\\"[^\\\"\\\\]*(\\\\.[^\\\"\\\\]*)*(ORDER BY|GROUP BY)" + WHITESPACE +
        // "\\\"(?=" + WHITESPACE + "\\+" + WHITESPACE + "[_a-zA-Z][_a-zA-Z0-9]*)";
        // public static final String STRING_LITERAL_CONCAT_WITH_TABLE =
        // "\\\"[^\\\"\\\\]*(\\\\.[^\\\"\\\\]*)*(FROM|INTO|UPDATE|JOIN|((create|alter|drop|rename|truncate)
        // TABLE))" + WHITESPACE + "\\\"(?=" + WHITESPACE + "\\+" + WHITESPACE +
        // "[_a-zA-Z][_a-zA-Z0-9]*)";
        // public static final String STRING_LITERAL_CONCAT_WITH_TABLE_LOWER =
        // "\\\"[^\\\"\\\\]*(\\\\.[^\\\"\\\\]*)*(FROM|INTO|UPDATE|JOIN|((create|alter|drop|rename|truncate)
        // TABLE))" + WHITESPACE + "\\\"(?=" + WHITESPACE + "\\+" + WHITESPACE +
        // "[a-z][_a-zA-Z0-9]*)";
        // public static final String STRING_LITERAL_CONCAT_WITH_COLUMN =
        // "\\\"[^\\\"\\\\]*(\\\\.[^\\\"\\\\]*)*(SELECT|SELECT DISTINCT|((alter|drop)
        // COLUMN)|WHERE)" + WHITESPACE + "\\\"(?=" + WHITESPACE + "\\+" + WHITESPACE +
        // "[_a-zA-Z][_a-zA-Z0-9]*)";

        // Unused regular expressions
        // public static final String STRING_LITERAL_CONCAT_WITH_TABLE = TABLE +
        // CONCAT_VAR;
        // public static final String STRING_LITERAL_CONCAT_MULTIPLE_WITH_TABLE = TABLE
        // + CONCAT_VAR_MULTIPLE;
        // public static final String STRING_LITERAL_CONCAT_WITH_GROUP_ORDER_BY =
        // GROUP_ORDER_BY + CONCAT_VAR;
        // public static final String STRING_LITERAL_CONCAT_MULTIPLE_WITH_GROUP_ORDER_BY
        // = GROUP_ORDER_BY + CONCAT_VAR_MULTIPLE;
}
