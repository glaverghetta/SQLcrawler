...
public class CodeAnalysisManager {
    ...
    // List of analyzers to run. Can remove or add new analzyers to list without
    // issue
    ...
    // private PreparedStatement mSelectStatement =
    // mConnection.prepareStatement("select project_name, commit_date, " +
    // "sql_usage, like_usage, api_type, file_url, raw_url from code_specs where
    // project_name = ?");
    ...
    public Analysis processFile(File f) {
        ...
        if (!SQLType.NONE.equals(result.getSql_usage())) {
            for (CodeAnalyzer analyzer : analyzers) {
                ...
                SQLType a = analyzer.analyzeCode(code, stringLiterals, language);
                long end = System.currentTimeMillis();

                timeLog.info("{} ~ {} ~ {} ~ {} ~ {} ~ {}", new Date(start), new Date(end), end - start,
                        analyzer.getDBField(), f.getId(), a);

                result.set(analyzer.getDBField(), a);
            }
        }
    ...
    // SQLType columnSQLType = mColumnNameCodeAnalyzer.analyzeCode(code,
    // stringLiterals);
    ...
    // SQLType tableLowerSqlType = mTableNameLowerAnalyzer.analyzeCode(code,
    // stringLiterals);
    ...
    // SQLType triggerSqlType = mTriggerNameAnalyzer.analyzeCode(code,
    // stringLiterals);
    ...
    // SQLType serverSqlType = mServerNameAnalyzer.analyzeCode(code,
    // stringLiterals);
    // analysisBuilder.setServerUsage(serverSqlType);

    // SQLType tSpaceSqlType = mTSpaceNameAnalyzer.analyzeCode(code,
    // stringLiterals);
